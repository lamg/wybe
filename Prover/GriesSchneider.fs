...
let ``GCD symmetry`` = ``∀`` [ m; n ] (gcd m n = gcd n m) |> axiom "GCD symmetry"

let ``GCD associativity`` =
  ``∀`` [ m; n; p ] (gcd (gcd m n) p = gcd m (gcd n p))
  |> axiom "GCD associativity"

let ``GS 15.98`` = ``∀`` [ n ] (gcd n n = Abs n) |> axiom "GS 15.98"

let ``gcd zero`` = ``∀`` [ n ] (gcd one n = one) |> axiom "GCD zero"

let ``GS 15.101`` =
  ``∀`` [ m; n ] (gcd m n = gcd (Abs m) (Abs n)) |> axiom "GS 15.101"

let ``GS 15.102`` = ``∀`` [ m; n ] (gcd m n = gcd m (m + n)) |> axiom "GS 15.102"
let ``GS 15.102a`` = ``∀`` [ m; n ] (gcd m n = gcd m (m - n)) |> axiom "GS 15.102a"
