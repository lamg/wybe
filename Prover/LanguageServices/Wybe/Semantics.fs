...
let rec exprToTree: Expr -> Core.SymbolTree =
  ...
  | Array elems ->
    ...
    | [] -> Core.SymbolTree.Node(Core.Symbol.Indexed, [])
    | x :: xs ->
      let commaTree =
        xs
        |> List.fold (fun acc e -> Core.SymbolTree.Node(Core.Symbol.Op(",", 0), [ e; acc ])) x

      Core.SymbolTree.Node(Core.Symbol.Indexed, [ commaTree ])
  | ArrayElem(name, index) ->
    Core.SymbolTree.Node(Core.Symbol.Atom name, [ Core.SymbolTree.Node(Core.Symbol.Indexed, [ exprToTree index ]) ])
